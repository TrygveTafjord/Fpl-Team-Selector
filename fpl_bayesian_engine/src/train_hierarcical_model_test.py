...
def prepare_full_dataset(master_df_path: Path, player_idlist_path: Path) -> Dict[str, Any]:
    ...
    # --- Step 1: Load the definitive list of ALL current players ---
    # This file is the single source of truth for who is in the model.
    try:
        df_players = pd.read_csv(player_idlist_path)
        # Use 'now_cost' for current season pricing, alias it to 'value' for consistency
        df_players['value'] = df_players['now_cost']
    ...
    # --- Step 2: Create Price Tiers for ALL current players ---
    ...
    df_players.loc[df_players['position'] == 'GK', 'price_tier'] = pd.cut(df_players['price'], bins=gkp_bins, labels=labels, right=False) # Note: 'GK' in playerlist
    ...
    # --- Step 3: Create the master mappings from the complete player list ---
    ...
    # --- Step 4: Load historical data and map it to our universe ---
    ...
    # NOTE: The position in historical data is 'GKP', not 'GK'. We standardize it here.
    df_hist['position'] = df_hist['position'].replace({'GKP': 'GK'})
    ...
    # --- Step 5: Define features and scale them based on historical data ---
    features = [
    ...
    # The returned maps now cover ALL players, while the data (X, y) is just for those with history.
    return {
        "X_scaled": X_scaled,
        "y": y.values,
